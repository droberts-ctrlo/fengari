...
const IO_INPUT = fengaricore.to_luastring(IO_PREFIX + 'input');
const IO_OUTPUT = fengaricore.to_luastring(IO_PREFIX + 'output');
...
const f_tostring = function(L) {
    ...
    if (isclosed(p))
        ...
        lua.lua_pushstring(L, fengaricore.to_luastring(`file (${p.f.toString()})`));
    return 1;
};
...
const tofile = function(L) {
    let p = tolstream(L);
    if (isclosed(p))
        lauxlib.luaL_error(L, fengaricore.to_luastring('attempt to use a closed file'));
...
const getiofile = function(L, findex) {
    ...
    if (isclosed(p))
        lauxlib.luaL_error(L, fengaricore.to_luastring('standard %s file is closed'), findex.subarray(IOPREF_LEN));
    return p.f;
};

const g_iofile = function(L, f, _mode) {
    if (!lua.lua_isnoneornil(L, 1)) {
        let filename = lua.lua_tostring(L, 1);
        if (filename)
            lauxlib.luaL_error(L, fengaricore.to_luastring('opening files not yet implemented'));
        else {
...
const g_write = function(L, f, arg) {
    ...
    for (; nargs--; arg++) {
        ...
        try {
            status = status && ((prepare_string_for_write(s).write()) === s.length);
        } catch (e) {
            status = false;
            err = e;
        }
...
const createmeta = function(L) {
    ...
    lua.lua_setfield(L, -2, fengaricore.to_luastring('__index', true));  /* metatable.__index = metatable */
...
export const luaopen_io = function(L) {
    lauxlib.luaL_newlib(L, iolib);
    createmeta(L);
    /* create (and set) default files */
    createstdfile(L, process.stdin, IO_INPUT, fengaricore.to_luastring('stdin'));
    createstdfile(L, process.stdout, IO_OUTPUT, fengaricore.to_luastring('stdout'));
    createstdfile(L, process.stderr, null, fengaricore.to_luastring('stderr'));
    return 1;
};
