...
import { LUA_REGISTRYINDEX, lua_getfield, lua_gettop, lua_isnone, lua_isnoneornil, lua_newuserdata, lua_pop, lua_pushliteral, lua_pushnil, lua_pushstring, lua_pushvalue, lua_setfield, lua_tostring, lua_touserdata } from './lua.js';
import { LUA_FILEHANDLE, luaL_checkany, luaL_checklstring, luaL_checkudata, luaL_error, luaL_fileresult, luaL_newlib, luaL_newmetatable, luaL_setfuncs, luaL_setmetatable, luaL_testudata } from './lauxlib.js';
...
const g_write = function(L, f, arg) {
    ...
    for (; nargs--; arg++) {
        ...
        try {
            status = status && (writeSync(f.fd, prepare_string_for_write(s), 0, s.length) === s.length);
        } catch (e) {
            status = false;
            err = e;
        }
...
const _luaopen_io = luaopen_io;
export { _luaopen_io as luaopen_io };
