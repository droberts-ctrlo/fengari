...
const constant_types = {
    LUA_TCCL, LUA_TLCF, LUA_TLCL, LUA_TNIL
};
const thread_status = {
    LUA_ERRMEM, LUA_ERRERR, LUA_ERRRUN, LUA_ERRSYNTAX, LUA_OK, LUA_YIELD
};
...
const tryfuncTM = function (L, off, func) {
    ...
    for (let p = L.top - 2; p > off; p--) lobject.setobjs2s(L, p, p - 1); /* move other items up one */
...
const luaD_rawrunprotected = function (L, f, ud) {
    ...
    let lj = {
        status: LUA_OK, previous: L.errorJmp /* chain new error handler */
    };
    L.errorJmp = lj;
...
const unroll = function (L, ud) {
    ...
    while (L.ci !== L.base_ci) {  /* something in the stack */
        if (!(L.ci.callstatus & lstate.CIST_LUA))  /* C function? */
            finishCcall(L, LUA_YIELD);  /* complete its execution */ else {  /* Lua function */
...
const resume = function (L, n) {
    ...
    if (L.status === LUA_OK) {  /* starting a coroutine? */
        ...
        if (ci.callstatus & lstate.CIST_LUA)  /* yielded inside a hook? */
            lvm.luaV_execute(L);  /* just continue running Lua code */ else {  /* 'common' yield */
...
const lua_resume = function (L, from, nargs) {
    ...
    if (status === -1)  /* error calling 'lua_resume'? */
        status = LUA_ERRRUN; else {  /* continue running after recoverable errors */
